  ...
  for (let p in list_data.types) {
    ...
    if (is_default) {
      for (let i = 0; i < list_data.types[p].length; i++) {
        ...
        exist_option = Array.prototype.some.call(select_architecture.options, opt => opt.value === operating_system.arch)
        if (!exist_option) {
        ...
        if (operating_system.file_name.endsWith('depot.gz')) {
          var file_type = 'depot.gz'
        } else {
          var file_type = operating_system.file_name.split('.').pop()
        }
